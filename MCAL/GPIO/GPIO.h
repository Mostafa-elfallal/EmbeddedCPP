...
namespace GPIO{
	...
	class GPIO_PORT {
		...
		static void writeDir(regType newVal) { DDR::write(newVal); }
	};
	
	template <typename gpio,const regType bitsMask>
	class GPIO_PIN{
		public:
			static const regType bitMask = bitsMask;
			...
			static void config(pinType conf) {
				switch (conf) {
